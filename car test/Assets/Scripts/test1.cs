...
public class test1 : MonoBehaviour
{
    ...
    public float lowTractionMultiplier;
    ...
    private void FixedUpdate()
    {
        if (isCarGrounded)
        {
            float traction = Mathf.Lerp(1f, lowTractionMultiplier, Mathf.Abs(turnInput) * currentSpeed / maxFwdSpeed);
            // Apply forward force with traction control
            sphereRB.AddForce(transform.forward * currentSpeed * traction, ForceMode.Acceleration);

            if (turnInput != 0)
            {
                // Apply lateral force to simulate traction loss during turns
                Vector3 lateralForce = transform.right * -turnInput * currentSpeed * (1f - traction);
                sphereRB.AddForce(lateralForce, ForceMode.Acceleration);
            }
            RaycastHit hit;
            if (Physics.Raycast(transform.position, -transform.up, out hit, 1f, groundLayer))
            {
                ...
                {
                    case "Gravel":
                        lowTractionMultiplier = 0.7f; // Example value for gravel
                        break;
                    case "Ice":
                        lowTractionMultiplier = 0.4f; // Example value for ice
                        break;
                    default:
                        lowTractionMultiplier = 1f; // Full traction on tarmac
                        break;
                }
            }
